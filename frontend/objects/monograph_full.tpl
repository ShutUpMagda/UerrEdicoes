...
<div class="obj_monograph_full">
	...
	{include file="frontend/components/breadcrumbs.tpl" currentTitleKey="plugins.themes.UerrEdicoes.breadcrumbMonograph"}
					...
					{foreach name="authors" from=$authors item=author}
						...
						{strip}
							...
							{if !$smarty.foreach.authors.last}
								...
								<!--Quebrando a linha depois da filiação-->
								<br/>
							{/if}
						{/strip}
					{/foreach}
					...
					{foreach from=$publication->getData('authors') item=author}
						{if $author->getLocalizedBiography()}
							<div class="sub_item">
								...
								{* Inserindo o código ORCID, caso exista *}
								{if $author->getOrcid()}
									<span class="orcid">
										<a href="{$author->getOrcid()|escape}" target="_blank">
											{$author->getOrcid()|escape}
